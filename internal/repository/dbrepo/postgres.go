...
func (m *postgresDBRepo) InsertRoomRestriction(r models.RoomRestriction) error {
	...
	_, err := m.DB.ExecContext(ctx, smtm,
		r.StartDate,
		r.EndDate,
		r.RoomID,
		r.ReservationID,
...
func (m *postgresDBRepo) SearchAvailabilityByDateByRoomID(start, end time.Time, roomID int) (bool, error) {
	...
	row := m.DB.QueryRowContext(ctx, smtm, roomID, start, end)
...
func (m *postgresDBRepo) SearchAvailabilityForAllRooms(start, end time.Time) ([]models.Room, error) {
	...
	query := `
			...
			from
				rooms r
			where r.id 
				not in (select rr.room_id from room_restrictions rr where $1 < rr.end_date and $2 > rr.start_date);`
